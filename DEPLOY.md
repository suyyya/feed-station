...
**2026 年的免费政策**（重要）：
- 注册就送 **$5 试用信用**（30 天有效）
- 试用结束后，Free 计划为 **$1/月**，同时包含 **$1/月 usage credit**；只要当月资源用量 ≤ $1，净账单就是 $0
- 完全 **不绑卡**
- **支持持久卷**（数据不会丢！）

对投喂站来说：一个轻量 Node 服务每月约 $0.80，$1 credit 刚好覆盖，所以**实际不花钱**。

**额度超限（用量 > $1/月）会发生什么**（2026-08 核实）：
- 容器**立即停止**，网站打不开，**无宽限期**
- Railway 会发邮件提醒你添加支付方式
- **数据不丢**：持久卷数据保留 **30 天**，30 天内充值/升级即可恢复访问
- **超过 30 天仍未处理** → 卷数据被**永久删除**（这才是真正"数据没了"）
- 恢复方式：升级 Hobby（$5/月）或添加支付方式后回到 Free，服务自动回线

所以结论：**服务会停，但数据有 30 天逃生窗口**，不会一超限就消失。
